...
fn build() -> (axum::Router, Arc<AppState>, TempDir, CancellationToken) {
    ...
    let sqlite = Arc::new(forward_server::store::Store::open(dir.path()).unwrap());
...
async fn v07_callers_get_array_root_with_v07_field_set() {
    ...
    for i in 0..4 {
        state.audit.push(AuditEntry {
            ...
            actor: format!("user-{i}"),
            role: Some(forward_auth::OperatorRole::User),
            method: "GET".into(),
            path: "/v1/users".into(),
            outcome: if i % 2 == 0 {
                AuditOutcome::Allow
...
async fn v07_outcome_filter_narrows_array() {
    ...
    state.audit.push(AuditEntry {
        ...
        actor: "u-allow".into(),
        role: Some(forward_auth::OperatorRole::Superadmin),
        method: "GET".into(),
        path: "/v1/users".into(),

//! 007-multi-target-failover T032 — `forward_rule_target_failovers_total`
...
/// `forward_rule_target_failovers_total`. The collector renders
...
async fn single_target_rules_emit_zero_failover_series() {
    ...
    let row_count = body
        .lines()
        .filter(|l| l.starts_with("forward_rule_target_failovers_total{"))
        .count();
    assert_eq!(
        row_count, 0,
...
async fn multi_target_failovers_emit_one_row_per_triple() {
    ...
    for (rid, owner) in rules {
        let pat = format!(
            "forward_rule_target_failovers_total{{client=\"edge-b\",owner=\"{owner}\",rule=\"{}\"}} 5",
            rid.0
        );
...
async fn mixed_fleet_only_failover_rules_emit_series() {
    ...
    assert!(
        body.contains(
            "forward_rule_target_failovers_total{client=\"edge-c\",owner=\"bob\",rule=\"34\"} 7"
        ),

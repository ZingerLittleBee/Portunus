...
//! down by `sni_route` integration tests in `forward-client`.
//!
//! References:
...
use forward_server::rules::{Protocol, RuleStoreError, ServerRuleStore};
...
async fn http_legacy_listener_refuses_sni_candidate() {
    ...
    let rule = f
        ...
        .push_range_with_targets(
            client.clone(),
            forward_core::PortRange::single(30030),
            "10.0.0.1".into(),
            forward_core::PortRange::single(9001),
            forward_server::rules::Protocol::Tcp,
            None,
            16,
            UserId::superadmin(),
            Vec::new(),

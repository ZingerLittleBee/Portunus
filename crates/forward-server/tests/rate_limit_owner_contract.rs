...
use forward_proto::v1::{Protocol as ProtoProtocol, server_message::Payload};
...
) -> tokio::sync::Mutex<tokio::sync::mpsc::Receiver<Result<forward_proto::v1::ServerMessage, tonic::Status>>>
{
    ...
    let waiters: Arc<
        tokio::sync::Mutex<HashMap<String, oneshot::Sender<forward_proto::v1::RuleStatus>>>,
...
async fn drain_owner_rate_limit_update(
    ...
    >,
) -> Option<forward_proto::v1::OwnerRateLimitUpdate> {

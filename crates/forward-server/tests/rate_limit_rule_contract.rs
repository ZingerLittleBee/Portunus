...
//! `forward-server::operator::http::post_rules`.
//!
//! Coverage:
...
//! only; data-plane enforcement is in `forward-client`.
    ...
    let waiters: Arc<
        tokio::sync::Mutex<
            std::collections::HashMap<
                String,
                tokio::sync::oneshot::Sender<forward_proto::v1::RuleStatus>,
            >,
        >,
    ...
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            ...
            if let Some(tx) = guard.remove(&request_id) {
                let _ = tx.send(forward_proto::v1::RuleStatus {
                    request_id,
                    rule_id,
                    outcome: forward_proto::v1::ActivationOutcome::Activated as i32,
                    reason: String::new(),
                });
            }
        }

...
//!   * T019: parse `proto/portunus.proto` and assert the field-number
...
/// Read the actual `proto/portunus.proto` file and assert that the
...
fn t019_proto_field_numbers_match_spec() {
    ...
    .expect("read proto/portunus.proto");
    ...
    for (needle, suffix) in &expected {
        ...
        assert!(
            found,
            "proto/portunus.proto missing line matching '{needle}' AND '{suffix}'\n\

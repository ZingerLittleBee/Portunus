...
use portunus_core::rate_limit::{effective_burst_u32, effective_burst_u64};
use portunus_core::{RateLimit, RejectReason, RuleId};
...
/// [`portunus_core::RateLimit`] envelope; immutable thereafter (a
...
impl RuleRateLimiter {
    ...
    /// invariant is enforced by `portunus_core::rate_limit::validate`
    ...
    pub fn bandwidth_rate_per_sec(&self, direction: BandwidthDirection) -> Option<u64> {
        ...
        bucket.map(TokenBucket::rate_per_sec)
    }
...
/// `portunus-client` rate-limit subsystem; one per process. All public
...
/// — the tenant-isolation scope per FR-002. The portunus-client

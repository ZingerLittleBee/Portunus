...
//!
//! Spawns a real `forward-server`, then exercises each subcommand
...
fn run_subcmd(args: &[&str]) -> std::process::Output {
    Command::new(common::workspace_bin("forward-server"))
        .args(args)
...
fn cli_user_credential_grant_roundtrip() {
    ...
    let out = Command::new(common::workspace_bin("forward-server"))
        ...
        .env("FORWARD_OPERATOR_TOKEN", &alice_token)
        .output()
        .expect("spawn rotate");
    assert!(
        out.status.success(),
...
fn cli_without_token_exits_4_unauthenticated() {
    ...
    let out = Command::new(common::workspace_bin("forward-server"))
        ...
        // Deliberately omit FORWARD_OPERATOR_TOKEN.
        .env_remove("FORWARD_OPERATOR_TOKEN")
        .output()
        .expect("spawn");
    assert_eq!(
        out.status.code(),

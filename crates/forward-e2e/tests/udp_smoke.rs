...
//!     `forward_rule_udp_datagrams_in_total` for the rule (SC-004).
...
/// `forward_rule_flows_dropped_overflow_total` counter MUST advance by
...
/// MUST emit exactly ONE row of `forward_rule_udp_datagrams_in_total`
...
fn test_udp_us3_metric_cardinality() {
    ...
    assert_eq!(
        ...
        "SC-004: range rule MUST emit exactly one row of \
         forward_rule_udp_datagrams_in_total (got {count}); /metrics:\n{body}"
    );
}

...
//! `portunus-server` + `portunus-client` pair using the existing
//! `portunus-e2e` harness.
//!
...
/// `portunus-client` will bind). Returns `(start, end)` inclusive.
...
/// - `/metrics` exposes one `portunus_rule_bytes_in_total` row for the
///   rule (label set is `(client, rule)` only — SC-002 cardinality
///   guarantee). Doubling the range size MUST NOT add Prometheus rows.
/// - `portunus-server rule-stats <id> --per-port` (the operator CLI)
...
fn test_range_us3_per_port_observability() {
    ...
    let bytes_in_rows = body
        .lines()
        .filter(|ln| ln.starts_with("portunus_rule_bytes_in_total{") && ln.contains(&needle))
        .count();
    assert_eq!(
        bytes_in_rows, 1,
        "SC-002: portunus_rule_bytes_in_total must have exactly 1 row for rule {rule_id}, got {bytes_in_rows}\n--- /metrics ---\n{body}"
    );

    // T038: CLI `rule-stats <id> --per-port` renders the per-port table.
    let cli_out = std::process::Command::new(common::workspace_bin("portunus-server"))
        .arg("rule-stats")
        ...
        .output()
        .expect("spawn portunus-server rule-stats");
    assert!(

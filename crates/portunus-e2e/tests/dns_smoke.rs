...
//!
//! Spins up `portunus-server` + `portunus-client`, pushes one rule with a
...
/// T045 (US4) — `portunus_rule_dns_failures_total` cardinality and
/// per-rule accuracy.
///
...
/// and asserts:
///   - exactly N `portunus_rule_dns_failures_total` rows (SC-006 /

...
fn test_prometheus_endpoint() {
    ...
    let body = common::wait_for(Duration::from_secs(12), || {
        ...
        if body.contains("portunus_rule_bytes_in_total{")
            && body.contains("portunus_auth_failures_total{")
        {
            Some(body)
        } else {
            None
...
fn test_user_story_3_acceptance() {
    ...
    let body = common::fetch_metrics_text(&metrics);
    assert!(
        body.contains("portunus_rule_bytes_in_total"),
        "metrics body: {body}"
    );
    assert!(
        body.contains("portunus_clients_connected"),
        "metrics body: {body}"
    );

    ...
    pub fn apply_push<R: Resolve + 'static>(
        ...
        match self.groups.get_mut(&listen_port) {
            ...
            None => {
                ...
                let listeners =
                    portunus_forwarder::forwarder::range::bind_all(&PortRange::single(listen_port))
                        .map_err(|err| {
...
//
// `portunus-client` is a bin-only crate today, so these can't live
...
mod e2e_tests {
    ...
    async fn t071_listener_counters_bump() {
        ...
        loop {
            ...
            let row = snap.iter().find(|r| r.listen_port == listen_port);
            if let Some(r) = row {

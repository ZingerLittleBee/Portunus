//! T042 (005-multi-user-rbac, US3) — `portunus_rule_*` per-rule
...
async fn per_rule_collectors_are_owner_labelled_and_one_row_per_triple() {
    ...
    for (rid, owner) in rules {
        let pat = format!(
            "portunus_rule_bytes_in_total{{client=\"edge-a\",owner=\"{owner}\",rule=\"{}\"}} 500",
            rid.0
        );
...
/// T045 second half: `portunus_operator_requests_total{outcome,reason}`
...
async fn operator_requests_counter_registered_and_renderable() {
    ...
    assert!(
        body.contains("portunus_operator_requests_total{outcome=\"allow\",reason=\"ok\"} 1"),
        "missing allow row\n--- body ---\n{body}"
    );
    assert!(
        body.contains(
            "portunus_operator_requests_total{outcome=\"deny\",reason=\"client_not_granted\"} 2"
        ),
        "missing deny row\n--- body ---\n{body}"
    );
}

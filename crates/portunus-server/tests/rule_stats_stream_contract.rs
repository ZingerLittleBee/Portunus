...
use portunus_server::rules::Protocol;
...
async fn build_harness() -> Harness {
    ...
    let bob_id = portunus_auth::UserId::from_str("bob").expect("user id");
...
async fn seed_rule(state: &AppState, owner: &str) -> RuleId {
    let owner_id = portunus_auth::UserId::from_str(owner).expect("uid");
    let rule = state
        .rules
        .push_range(
            ...
            "127.0.0.1".to_string(),
            portunus_core::PortRange::single(9000),
            Protocol::Tcp,
            None,
            16,
            owner_id,

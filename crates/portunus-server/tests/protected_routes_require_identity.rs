...
fn operator_sources() -> Vec<String> {
    let mut out = Vec::new();
    for entry in fs::read_dir(operator_dir()).expect("read src/operator") {
        let path = entry.expect("dir entry").path();
        if path.extension().and_then(|e| e.to_str()) == Some("rs") {
            out.push(fs::read_to_string(&path).expect("read source file"));
        }
    }
    out
}
...
/// Whether the first `async fn <name>(...)` found in the corpus takes an
/// `OperatorIdentity` parameter. `None` if no such fn is found.
fn takes_identity(sources: &[String], name: &str) -> Option<bool> {
        ...
        for (j, &c) in src.as_bytes().iter().enumerate().skip(open) {
            match c {
                ...
                b')' => {
                    depth -= 1;
                    if depth == 0 {
                        return Some(src[open..=j].contains("OperatorIdentity"));
                    }
                }
                _ => {}
            }

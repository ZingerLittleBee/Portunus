...
//! portunus-client `tracing` only and are observable via the structured
...
use portunus_server::operator::audit::{AuditEntry, AuditOutcome, AuditRing};
use std::sync::Once;
...
fn data_plane_sni_events_do_not_leak_into_audit_ring() {
    ...
    // the exact targets emitted by `portunus-client/src/forwarder/sni`.

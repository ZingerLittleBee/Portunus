...
use portunus_auth::ClientIdentity;
    ...
    async fn channel(
        ...
        let pending_first_msg: Option<ClientMessage> = match inbound.next().await {
            Some(Ok(client_msg)) => match &client_msg.payload {
                Some(portunus_proto::v1::client_message::Payload::Hello(h)) => {
...
async fn handle_client_message(
    ...
    use portunus_core::RuleId;
    use portunus_proto::v1::client_message::Payload;
    match msg.payload {
            ...
            for entry in report.stats {
                ...
                if let Some(rl) = entry.rate_limit.as_ref() {
                    ...
                    for c in &rl.reject_total {
                        if let Ok(reason) =
                            portunus_proto::v1::RateLimitRejectReason::try_from(c.reason)
                        {
                            let idx = match reason {
                                portunus_proto::v1::RateLimitRejectReason::ConnConcurrent => 0,
                                portunus_proto::v1::RateLimitRejectReason::ConnRate => 1,
                                portunus_proto::v1::RateLimitRejectReason::UdpFlowRate => 2,
                                portunus_proto::v1::RateLimitRejectReason::OwnerConcurrent => 3,
                                portunus_proto::v1::RateLimitRejectReason::OwnerConnRate => 4,
                                portunus_proto::v1::RateLimitRejectReason::OwnerUdpFlowRate => 5,
                                // Unspecified is the proto default; the
                                // client never emits it.
                                portunus_proto::v1::RateLimitRejectReason::Unspecified => continue,
                            };
...
fn replay_gate_reason(
    ...
    // carrying any rate-limit cap to a portunus-client whose
...
fn proto_rule_from_rule(rule: &crate::rules::Rule) -> Rule {
    Rule {
        ...
        targets: rule
            .targets
            .iter()
            .map(|target| portunus_proto::v1::Target {
                host: target.host.clone(),
                port: u32::from(target.port),
                priority: target.priority,
                proxy_protocol: target.proxy_protocol.map(|mode| match mode {
                    portunus_core::ProxyProtocolVersion::V1 => ProxyProtocolVersion::V1 as i32,
                    portunus_core::ProxyProtocolVersion::V2 => ProxyProtocolVersion::V2 as i32,
                }),
            })
            .collect(),
        ...
        // 011-rate-limiting-qos: always surface owner_id on the wire
        // so owner-cap updates can bind to already-active uncapped
        // rules without a rule re-push. Older clients ignore the
        // additive field on decode.
        owner_id: Some(rule.owner_user_id.to_string()),
    }
}

/// 011-rate-limiting-qos T007: encode a `portunus_core::RateLimit` into
/// the wire-shape `portunus_proto::v1::RateLimit`. Field tags are
/// 1-1 with the `RateLimit` definition in `proto/portunus.proto` and
/// every cap is independently optional.
pub(crate) fn rate_limit_to_proto(rl: &portunus_core::RateLimit) -> portunus_proto::v1::RateLimit {
    portunus_proto::v1::RateLimit {
...
pub(crate) fn rate_limit_from_proto(p: &portunus_proto::v1::RateLimit) -> portunus_core::RateLimit {
...
async fn apply_unsolicited_rule_status(
    state: &AppState,
    identity: &ClientIdentity,
    status: &portunus_proto::v1::RuleStatus,
) {
    ...
    let outcome = portunus_proto::v1::ActivationOutcome::try_from(status.outcome)
        .unwrap_or(portunus_proto::v1::ActivationOutcome::Unspecified);
    match outcome {
        portunus_proto::v1::ActivationOutcome::Activated => {
            let _ = state.rules.mark_active(rule_id).await;
        }
        portunus_proto::v1::ActivationOutcome::Failed => {
        ...
        portunus_proto::v1::ActivationOutcome::Removed
        | portunus_proto::v1::ActivationOutcome::Unspecified => {
            info!(
                event = "client.rule_status_unmatched",
                client_name = %identity.client_name,
                request_id = %status.request_id,
...
mod tests {
    ...
    // `portunus-e2e`; here we lock down the pure helpers that decide
    ...
    async fn replay_rules_sends_proxy_protocol_targets_to_capable_client() {
        ...
        let rule = crate::rules::Rule {
            ...
            owner_user_id: portunus_auth::UserId::reserved("alice"),
    ...
    async fn unsolicited_rule_status_ignores_rules_owned_by_other_client() {
        ...
        let other_client = portunus_core::ClientName::new("edge-b".to_string()).unwrap();
        let now = chrono::Utc::now();
        state
            .rules
        ...
        apply_unsolicited_rule_status(
            &state,
            &identity,
            &portunus_proto::v1::RuleStatus {
                request_id: "stale-or-forged".into(),
                rule_id: 11,
                outcome: portunus_proto::v1::ActivationOutcome::Failed as i32,
    ...
    fn cap_envelope() -> portunus_core::RateLimit {
        portunus_core::RateLimit {

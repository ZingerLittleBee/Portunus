...
use portunus_server::OutputFormat;
use portunus_server::clients::ConnectedClients;
use portunus_server::operator::bootstrap;
...
use portunus_server::tls::ServerTlsMaterial;
...
enum Cmd {
    ...
    /// directory, writes `portunus-state-<RFC3339>.db` inside it.
    Backup {
        #[arg(long)]
        out: PathBuf,
    },
...
fn run(cli: Cli) -> Result<(), u8> {
    ...
    match cli.cmd {
        ...
        Cmd::Backup { out } => match portunus_server::store::backup::run_backup(&data_dir, &out) {
            Ok(written) => {
        ...
        Cmd::Restore { r#in, force } => {
            ...
            match portunus_server::store::backup::run_restore(&r#in, &data_dir, force) {
                Ok(()) => {
        ...
        Cmd::Audit(AuditCmd::Prune { before, dry_run }) => {
            ...
            let store = match portunus_server::store::Store::open(&data_dir) {
        ...
        Cmd::Reset { confirm } => {
            ...
            match portunus_server::store::backup::run_reset(&data_dir) {
                Ok(()) => Ok(()),
                Err(e) => {
                    eprintln!("error: {e}");
                    Err(e.exit_code())

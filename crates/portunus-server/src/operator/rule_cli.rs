...
/// `PORTUNUS_OPERATOR_TOKEN` env var (set by the operator's shell, by
/// e2e tests, or by `bootstrap-superadmin --print-export`). The legacy
/// `PORTUNUS_OPERATOR_TOKEN` name is still accepted for existing scripts.
...
fn render_rate_limit_summary(rl: Option<&portunus_core::RateLimit>) -> String {
    let Some(rl) = rl else {
        return "-".to_string();
    };

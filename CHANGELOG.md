...
user-facing operator-API-token surface — the `POST/GET/DELETE
/v1/users/{id}/credentials` routes, the Web UI Credentials section, and
the `credential-*` server CLI subcommands — is gone. Machine and API
access now go through the static superadmin `operator_token`; humans use
Web cookie/password login. The data-plane client enrollment /
credential-bundle system is untouched. See **Removed** / **Changed** /
**Kept** below.

The `install.sh` lifecycle manager also becomes a pure flag-driven CLI.
The interactive menu, the guided install wizard, and the bilingual
(en/zh) output are removed; every action is now selected by arguments,
and all output is English. The installer's executor — binary/Docker
install, service lifecycle, scoped config rendering, and Caddy/HTTPS
setup — is unchanged.

### Added
- **`user-add --password-stdin` (server CLI).** Creating a user now
  requires an initial password; the `user-add` subcommand reads it from
  the first stdin line, mirroring `reset-password --password-stdin`.
...
### Changed
- **Breaking: creating a user requires an initial password.** Both the
  HTTP route and the `user-add` CLI now reject a request with no password
  (`422 initial_password_required`); the Web create-user form requires a
  password of at least 12 characters.
- **Breaking: password reset revokes only Web sessions.** Resetting a
  user's password (admin reset and the `reset-password` CLI) now
  invalidates only that user's active Web sessions and no longer revokes
  any API tokens — there are no per-user API tokens left to revoke.
- **Breaking: V013 migration purges user-issued credentials.** Upgrading
  runs the V013 migration, which deletes every existing user-issued
  credential row. The reserved `_legacy` and `_superadmin` bootstrap
  credentials are preserved, so superadmin login and the static
  `operator_token` keep working across the upgrade.
...
### Removed
- **Breaking: the user operator-API-token surface.** Removed the
  `POST/GET/DELETE /v1/users/{id}/credentials` and
  `.../credentials/{credId}/rotate` HTTP routes; the `credential-issue`,
  `credential-list`, `credential-revoke`, and `credential-rotate` server
  CLI subcommands; the Web UI Credentials section; and the
  `keep_api_tokens` (password-reset request) / `api_tokens_revoked`
  (password-reset response) / `credential_count` (user view) wire fields,
  along with the `--keep-api-tokens` flag on the `reset-password` CLI.
...
### Kept
- **Machine/API auth and human login are unchanged in shape.** The static
  `operator_token` (set in `server.toml`, generated by `gen-token`)
  remains the way machines and API clients authenticate, alongside
  `bootstrap-superadmin`; humans keep Web cookie/password login. The
  data-plane client enrollment / credential-bundle system is unaffected
  by this change.

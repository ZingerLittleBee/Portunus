...
- [x] T006 [P] Contract test for `--data-dir` resolution order in `/Users/zingerbee/Documents/forward-rs/crates/forward-server/tests/data_dir_resolution.rs`: cover (a) `--data-dir` explicit override; (b) `$STATE_DIRECTORY` env var; (c) `$XDG_STATE_HOME/portunus`; (d) `$HOME/.local/state/portunus`; (e) cwd `./portunus.state` fallback. Asserts the resolved path matches FR-019.

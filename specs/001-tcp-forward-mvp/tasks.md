...
- [X] T068 Run `quickstart.md` end-to-end on two real Linux hosts; capture wall-clock time for SC-001 (target < 5 minutes) — **verified 2026-05-06 on Debian 13 (trixie) x86_64 with musl static binaries cross-compiled from macOS via `cargo zigbuild`. T0 → first byte through rule = 1.262 s (server.listening 0.224, provisioned 0.473, client connected 0.968, rule pushed 1.026, first byte 1.262). Single Linux host (server + client co-located) instead of two; gRPC still over real socket + real TLS handshake. After 5×curl through the rule, `rule-stats` reports bytes_in=450 / bytes_out=5052 and `/metrics` exposes matching `forward_rule_bytes_{in,out}_total{client="edge-01",rule="0"}`. Far under the 300 s SC-001 target.**

...
- [ ] T014 [P] [US1] forward-client criterion bench: data-plane no-cap regression (≤ 2% throughput / ≤ 5% setup-latency vs v0.10) in [crates/forward-client/benches/rate_limit_overhead.rs](/Users/zingerbee/Documents/forward-rs/crates/forward-client/benches/rate_limit_overhead.rs) — SC-004 gate. **Status**: deferred. The crate is binary-only (no `[lib]` target — established convention since v0.1.0), so a criterion bench cannot import `forward_client::forwarder::rate_limit` internals to compare the v0.11 wrapped path against the v0.10 `tokio::io::copy_bidirectional` baseline. Adding `[lib]` is a workspace-wide convention change out of scope for v0.11. SC-004 is instead enforced by:
   - The wire byte-stability gate (`rate_limit_wire_compat` — v0.10 rules with no caps serialise byte-for-byte identically), AND
   - The data-plane short-circuit at [`failover_path.rs:424`](/Users/zingerbee/Documents/forward-rs/crates/forward-client/src/forwarder/failover_path.rs) that picks the v0.10 `tokio::io::copy_bidirectional` path when both `rule_has_bw` and `owner_has_bw` are false (no atomic touched on the no-cap accept), AND
...
- [ ] T043 Run quickstart.md end-to-end on a fresh build (`cargo run` server + client; exercise all 9 steps). **Status**: deferred. **Coverage matrix**:
   - Step 1 (start server + v0.11 client): exercised by every forward-e2e harness test.
   - Step 2 (push capped TCP rule + verify `forward_rate_limit_*` metrics): exercised by `rate_limit_rule_contract.rs` (PUT cap → 200; metrics emitted by `metrics.rs::observe_rate_limit_per_owner` covered by unit test).

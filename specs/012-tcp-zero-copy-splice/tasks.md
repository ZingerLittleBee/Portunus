...
- [ ] T006 [US1] Unit test for `eligible(&CopyCtx)` truth table — 7 cases per [contracts/internal-api.md § §1](./contracts/internal-api.md) (no caps, rule bandwidth_in_bps set, rule bandwidth_out_bps set, only concurrent_connections set, only new_connections_per_sec set, owner bandwidth set, disable_splice=true). Lives in `#[cfg(test)] mod eligible_tests` inside [crates/portunus-client/src/forwarder/splice.rs](/Users/zingerbee/Documents/forward-rs/crates/portunus-client/src/forwarder/splice.rs). The test asserts the function does not heap-allocate (counter-based or `dhat`/`heaptrack`-style mini-harness).
...
- [ ] T010 [US1] Integration test `t015_disable_splice_in_ctx_forces_userspace_path` in `#[cfg(test)] mod eligible_tests` inside [crates/portunus-client/src/forwarder/splice.rs](/Users/zingerbee/Documents/forward-rs/crates/portunus-client/src/forwarder/splice.rs) — build `CopyCtx { disable_splice: true, .. }`, call `eligible(&ctx)`, assert it returns `false`. (Pair test for FR-004.)
- [ ] T011 [US1] Integration test `t016_bandwidth_cap_in_ctx_forces_userspace_path` — same as T010 but with `has_bandwidth_cap: true`. Establishes that `eligible()` returns false; caller (proxy.rs) is responsible for actually taking the fallback branch.

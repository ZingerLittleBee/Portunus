...
- [X] T017 [US1] Wired splice into [crates/portunus-client/src/forwarder/proxy.rs](/Users/zingerbee/Documents/forward-rs/crates/portunus-client/src/forwarder/proxy.rs) via a new helper `copy_uncapped(inbound, outbound, rule_id, rate_limit, owner_rate_limit, has_sni_replay_done, has_proxy_out)`. The helper is called in the `else` branch of the `rule_has_bw || owner_has_bw` guard (i.e. the existing uncapped fast path) immediately after SNI peek+replay (preread write) and PROXY-out prelude have completed. On Linux, builds `CopyCtx::build`, calls `splice::eligible`; on success, returns `splice::copy_bidirectional` result mapped to `(bytes_in, bytes_out)`. On `Err(SpliceError::Unsupported { errno })` emits `proxy.splice_unsupported_fallback` (warn) and falls through to userspace `copy_bidirectional_with_sizes`. On `Err(SpliceError::Io)` returns the error (no retry — bytes may have moved). On non-Linux the helper is a thin wrapper over `copy_bidirectional_with_sizes`. **Live and verified on Linux**: full forwarder test suite (166 tests) passes under both `splice on` and `PORTUNUS_DISABLE_SPLICE=1` (SC-005 client-side gate green).
- [X] T018 [US1] All three tracing events landed at their final sites:
  ...
  - `proxy.splice_unsupported_fallback` (warn) — emitted by the `Err(SpliceError::Unsupported)` arm in `proxy::copy_uncapped` (T017). Fields: `rule_id`, `errno_name`.
- [X] T019 [US1] Byte-counter accounting is wired by T017's `copy_uncapped` helper: it maps `splice::copy_bidirectional`'s `Transferred { bytes_in, bytes_out }` directly to the same `(u64, u64)` shape the existing `copy_bidirectional_with_sizes` returns, so the downstream `record_in(listen_port, *bin + preread_in)` / `record_out(listen_port, *bout)` accounting (proxy.rs:308) is unchanged and the Prometheus / SQLite stats path is bit-identical between the two backends. No new metric (see [contracts/internal-api.md § §4](./contracts/internal-api.md)). SC-004 metric continuity is enforced byte-for-byte by `t012_byte_counters_match_userspace_path`.
- [X] T020 [P] [US1] Implement criterion bench `splice_throughput.rs` at [crates/portunus-client/benches/splice_throughput.rs](/Users/zingerbee/Documents/forward-rs/crates/portunus-client/benches/splice_throughput.rs). Reproduces the proxy primitive shape in-bench (R-009, mirroring `data_plane.rs` pattern). Four bench groups: `plain_tcp_1mib_chunks/splice_on`, `plain_tcp_1mib_chunks/splice_off` (sets `PORTUNUS_DISABLE_SPLICE=1` via `Criterion`'s env), `sni_routed_1mib_chunks/splice_on`, `sni_routed_1mib_chunks/splice_off`. Bench registered in [crates/portunus-client/Cargo.toml](/Users/zingerbee/Documents/forward-rs/crates/portunus-client/Cargo.toml) `[[bench]]` table.
...
- [X] T024 [US2] **Verified by code structure**: in `proxy_with_preread_and_prelude`, the prelude write (`proxy_protocol::write_prelude` ~line 235) and the SNI ClientHello replay (`outbound.write_all(buf)` ~line 248) BOTH execute before the `let result = tokio::select! { ... }` block at line 262 that contains the splice call. Therefore `copy_uncapped` (T017) only runs after both v0.9 and v0.10 preludes have flushed — splice never observes the prelude bytes. `has_sni_replay_done = preread_in > 0` and `has_proxy_out = had_proxy_prelude` are propagated for tracing-correlation only. No code change required.
...
- [X] T028 [P] [US3] Integration test `t024_hot_remove_bandwidth_cap_promotes_new_connections_to_splice` (server-side, the operator HTTP path) in [crates/portunus-server/tests/splice_hot_reload.rs](/Users/zingerbee/Documents/forward-rs/crates/portunus-server/tests/splice_hot_reload.rs) (new file). Push rule with `bandwidth_in_bps`, open one connection (in-flight on userspace path), then `PUT /v1/rules/{id}` removing the cap. Assert: existing connection completes on userspace path (FR-005); a fresh connection opened after the PUT uses splice (verified via `tracing-test` capture of `proxy.splice_selected`).
- [~] T029 [P] [US3] **Partially landed.** Manual verification on the dedicated Linux bench host (Debian 13 / kernel 6.12 / glibc 2.41): the full `cargo test forwarder::` suite passes **166/166 with splice on AND 166/166 with `PORTUNUS_DISABLE_SPLICE=1`**, including all v0.11 rate-limit tests. The CI matrix axis (a second `cargo test --workspace` job with the env var set in `.github/workflows/`) is deferred to a follow-up commit since it touches a YAML file the local worktree's CI config doesn't directly own. SC-005 gate is green for the v0.11 invariants; CI-axis automation will track in the PR.
...
- [X] T033 [P] Run `cargo bench --bench splice_throughput -- --baseline v1.2.0` on the dedicated perf host. Confirm: `plain_tcp_1mib_chunks/splice_on` throughput ≥ baseline × 1.4 (SC-001), p99 setup latency within ±5 % of baseline (SC-002), `*_splice_off` numbers match baseline within Criterion noise floor (kill-switch sanity).

- [X] T034 [P] Fallback resilience contract for SC-007 — landed as `mod classify_tests` inside `mod linux` of [crates/portunus-client/src/forwarder/splice.rs](/Users/zingerbee/Documents/forward-rs/crates/portunus-client/src/forwarder/splice.rs). 9 unit tests directly exercise `classify(errno, moved_any)` — the central fallback-decision function — verifying: (a) each of ENOSYS / EINVAL / EPERM / EOPNOTSUPP before first byte returns `SpliceError::Unsupported`; (b) EIO and ECONNRESET always return `SpliceError::Io`; (c) ENOSYS and EPERM AFTER any byte has moved promote to `SpliceError::Io` (no fallback — bytes in flight); (d) classify reads but never writes `moved_any`. An integration-level injection test was attempted with a thread-local one-shot errno injector inside `splice_raw`, but is racy against tokio's task scheduler even on the `current_thread` runtime — the unit-level contract on `classify` is the canonical guarantee. The `proxy::copy_uncapped` fallback arm (T017) is exercised on the real Linux kernel by the 175-test forwarder suite.

- [X] T035 [P] Cross-platform compile-out sanity check (SC-006) — verified on this branch:

  ```sh
  $ cargo build --release -p portunus-client            # darwin host, succeeded
  $ nm target/release/portunus-client | grep -ic 'pipe_pair\|splice_dir\|copy_bidirectional$\|drain_pipe'
  0
  ```

  Acceptance: build succeeds and all Linux-only splice symbols (PipePair, splice_dir, drain_pipe_to, the inner `copy_bidirectional`) are absent from the darwin binary — `#[cfg(target_os = "linux")]` gates worked. The cross-platform shell types (`CopyCtx`, `eligible` stub returning `false`, `Transferred`, `SpliceError`) intentionally remain present on every platform per the module's design.

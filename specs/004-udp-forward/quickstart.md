...
- `forward-server` and `forward-client` v0.4.0 binaries on `$PATH`.
...
### 2. Provision and start forward-server + client (one-host)
...
```sh
forward-server push-rule edge-01 6000 127.0.0.1:9999 --protocol udp
...
`forward-client` log shows a `rule.udp_flow_opened` event for the
new flow.
...
```sh
forward-server push-rule edge-01 6001 udp.test:9999 --protocol udp
...
forward-server rule-stats 0 | grep active_flows
# → active_flows=10
...
```sh
curl -s 127.0.0.1:7081/metrics | grep -E 'forward_rule_(active_flows|udp_datagrams|flows_dropped_overflow)'
# → forward_rule_active_flows{client="edge-01",rule="0"} 0
# → forward_rule_udp_datagrams_in_total{client="edge-01",rule="0"} 12
# → forward_rule_udp_datagrams_out_total{client="edge-01",rule="0"} 12
...
forward-server rule-stats 0 | grep datagrams

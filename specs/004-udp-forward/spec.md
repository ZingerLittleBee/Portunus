...
- **The TCP "active_connections" gauge and the UDP "active_flows" gauge are split at both wire and Prometheus layers**: distinct `RuleStats` proto fields (`active_connections = 4` TCP-only, `active_flows = 9` UDP-only — see `contracts/portunus.proto`) and distinct collectors (`portunus_rule_active_connections` and `portunus_rule_active_flows`). Operator surfaces (`rule-stats` text, JSON) pick the appropriate field based on the rule's `protocol`. (Rationale in `research.md` § R-007.)
- **Wire-format additivity**: every wire delta in this feature is additive — `Protocol::UDP = 2` joins the existing enum, `Hello.supported_protocols = 3`, `Welcome.udp_flow_idle_secs = 3` / `udp_max_flows_per_rule = 4`, four new `RuleStats` UDP fields (7-10), two new `PerPortStats` UDP fields (4-5). Proto3 default-zero + unknown-field-drop semantics mean a v0.3.0 reader receiving a v0.4.0 message ignores the new fields and a v0.4.0 reader receiving a v0.3.0 message sees default-zero everywhere. See `contracts/portunus.proto` for field-by-field rationale.

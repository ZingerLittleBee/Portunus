...
- [~] T031 [P] [US3] Per-target stats contract test deferred: surface coverage is exercised by Phase 6's wire-through-CLI e2e (T040) and by the existing rule_stats path tests (range smoke / observability suite). The structural invariant (single-target rules emit `per_target: []`) is enforced by `RuleSlot::multi_target_obs.is_none()` in `build_per_target` and by `#[serde(skip_serializing_if = "Vec::is_empty")]` on `RuleStatsSnapshot::per_target`.
- [~] T032 [P] [US3] Cardinality test deferred — the SC-006 invariant is structural: `RuleStatsCache::observe_with_targets` only inc's `rule_target_failovers_total` when the delta is `> 0`. Single-target rules pass `target_failovers_total: 0` forever so the delta stays 0 and no series is created. Verified by code inspection.
...
- [X] T034 [US3] TCP per-target byte accumulation: `failover_path::handle_connection` credits the chosen target's `HealthState.add_bytes_in/out` after `copy_bidirectional` returns, alongside the per-port `RuleStats.record_in/out`. Per-target connection counter incs on accept (`HealthState::increment_connections_accepted`). UDP per-target byte counters require threading the chosen target idx through `UdpFlow` and are deferred to a follow-up.
...
- [~] T037 [US3] SSE stream `?per_target=true` deferred — the existing `/v1/rules/{id}/stats/stream` already serialises `RuleStatsSnapshot` so per-target snapshots ride through automatically once `?per_target=true` is plumbed; the query-param plumbing is the same shape as T036 and is straightforward but skipped for v0.7.0 to keep scope tight.
- [~] T038 [US3] `targets[]` injection on `GET /v1/rules/{id}` deferred — the rule store already serialises `Rule.targets` (with `skip_serializing_if = "Vec::is_empty"`); operators can read the live target list via the stats endpoint with `?per_target=true`. Adding `health` snapshots to the rules list response is straightforward but skipped for v0.7.0.

...
- `crates/portunus-proto/` — wire protocol
- `crates/portunus-core/` — shared types
- `crates/portunus-server/` — operator HTTP, CLI, persistence, gRPC server
- `crates/portunus-client/` — gRPC client, forwarder, health/failover
...
- [X] T004 Write proto contract test `crates/portunus-proto/tests/targets_wire_compat.rs` covering W-1..W-6 from `contracts/proto-rule-extension.md` (legacy round-trip, v0.6 reader drops field 9 silently, byte-eq for single-target, RuleStats back-compat). MUST FAIL before T005/T006.
- [X] T005 Extend `proto/portunus.proto`: add `message Target`, `repeated Target targets = 9` and `uint32 health_check_interval_secs = 10` on `Rule`, `uint64 target_failovers_total = 11` and `repeated PerTargetStats per_target = 12` on `RuleStats`, and new `message PerTargetStats` per `contracts/proto-rule-extension.md` §1–§4
- [X] T006 Re-run `cargo build -p portunus-proto` to regenerate the tonic types and confirm T004 passes (6/6 wire-compat tests green; back-compat shimmed in 3 downstream sites with `targets: vec![]` / `health_check_interval_secs: 0` and `target_failovers_total: 0` / `per_target: vec![]` defaults)

### Core entity types

- [X] T007 [P] Create `crates/portunus-core/src/rule_target.rs` with `pub struct RuleTarget { host: String, port: u16, priority: u32 }` plus `pub fn rule_target::validate(targets: &[RuleTarget]) -> Result<(), RuleTargetError>` enforcing V-T1..V-T4 + V-R5. (NB: name is `RuleTarget` not `Target` — `portunus_core::Target` is already taken by the existing host-classifier module.)
- [X] T008 [P] Add `crates/portunus-core/tests/rule_target_validation.rs` with 13 cases: empty/single/max accepted, 9-target rejected, empty-host, invalid-host syntax, port 0, dup (host,port), same-host-different-port accepted, same-priority-value accepted, IPv4/bracketed-IPv6 accepted, unbracketed IPv6 rejected
- [X] T009 Extend `crates/portunus-server/src/rules.rs` (canonical `Rule` lives here, NOT in portunus-core): add `targets: Vec<RuleTarget>` and `health_check_interval_secs: Option<u32>` with `#[serde(default, skip_serializing_if = ...)]` so legacy on-disk shapes round-trip; add `targets_view()` and `is_multi_target()` helpers
- [X] T010 Re-export `RuleTarget`, `RuleTargetError`, `MAX_TARGETS_PER_RULE` from `crates/portunus-core/src/lib.rs`

### Persistence read-tolerance

- [~] T011 ~~Update `crates/portunus-server/src/persistence.rs` write path~~ **N/A**: rules are in-memory only in this codebase ("future work" per `crates/portunus-server/src/rules.rs:42`). The new `Rule.targets` and `Rule.health_check_interval_secs` fields carry serde-default + skip-if-empty attributes so back-compat is automatic if persistence is ever added.
...
- [X] T027 [P] [US2] `crates/portunus-e2e/tests/multi_target_recovery.rs`: pushes a 2-target rule with `health_check_interval_secs: 1`, validates initial routing to primary, kills primary and confirms failover to secondary, then best-effort re-binds primary and confirms recovery via the active probe + new-connect routing.
...
- [X] T031 [P] [US3] Per-target stats contract test in `crates/portunus-server/tests/rules_per_target_stats.rs` (3 tests, all green): single-target snapshots strip `per_target` from the wire and carry `target_failovers_total: 0`; multi-target snapshots round-trip the per-target body verbatim (host/port/priority, health flag, consecutive_failures, last-failure/success unix-ms timestamps, byte/connection counters); cumulative `target_failovers_total` advances monotonically across observations.
- [X] T032 [P] [US3] Cardinality test in `crates/portunus-server/tests/metrics_cardinality.rs` (3 tests, all green): single-target rules contribute zero rows to `portunus_rule_target_failovers_total`; multi-target rules emit exactly one row per `(client, rule, owner)` triple; mixed-fleet (4 single + 1 multi) produces exactly 1 row, mirroring the SC-006 cardinality budget.

### Implementation for User Story 3

- [X] T033 [US3] `send_stats_report` in `crates/portunus-client/src/control.rs` populates `target_failovers_total` from the per-rule `AtomicU64` (held in `RuleSlot.multi_target_obs`) and `per_target[]` from each per-target `HealthState` snapshot via the new `build_per_target` helper. Single-target rules emit `0` + `[]` (I-3).
...
- [X] T052 `crates/portunus-server/src/operator/audit.rs` module doc now calls out the correlation contract for `event = "rule.target.health_changed"` log lines: client-side origin, NOT pushed to the audit ring (would burn drop-oldest budget under sustained instability), correlate via `rule_id` against operator allow events the ring DOES capture.
...
Task: "Write Healthy→Failed state machine contract test in crates/portunus-client/tests/failover_state_contract.rs"
Task: "Write passive-failover e2e in crates/portunus-e2e/tests/multi_target_passive_failover.rs"
Task: "Write selection algorithm tests in crates/portunus-client/tests/selection_algorithm.rs"
```

```bash
# Then land US1 implementation in two parallel arms after T022:
Task: "Implement TCP failover loop in crates/portunus-client/src/forwarder/mod.rs (multi-target arm)"

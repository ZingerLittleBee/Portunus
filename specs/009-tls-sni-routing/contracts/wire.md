...
message Rule {
  ...
  // plain-TCP rule. The portunus-server capability gate (sni_unsupported_by_client,
...
A v0.9 portunus-client MUST declare its version in the existing `Hello`
...
The portunus-server gates SNI rule pushes against this value (see
...
Tests live in `crates/portunus-proto/tests/`.
...
| `sni_wire_compat.rs::wire_layout_documented` | Reads this very file and asserts the field-number registry table matches the actual `portunus.proto` declarations (uses `prost-build` reflection or string-grep the proto file). Prevents drift. |

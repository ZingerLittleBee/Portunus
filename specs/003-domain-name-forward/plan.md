...
- One new metric `portunus_rule_dns_failures_total{client,rule}` joins the
...
**Storage**: existing rules persistence layer (`portunus-server`'s
...
**Constraints**:
  ...
  - Prometheus cardinality: one row per rule for the new
    `portunus_rule_dns_failures_total` (SC-006).
...
├── contracts/
│   ├── portunus.proto    # Phase 1: additive proto diff (overlay vs v0.2.0)
...
```text
proto/
└── portunus.proto                         # add optional bool prefer_ipv6 = 8 on Rule

crates/
├── portunus-core/src/
│   ├── hostname.rs                       # NEW: RFC 1123 strict-hostname validator + Target enum (Ip|Dns)
│   └── lib.rs                            # re-export
├── portunus-proto/                        # codegen consumes the additive .proto
...
│   └── metrics.rs                        # NEW counter: portunus_rule_dns_failures_total
├── portunus-client/src/
│   ├── resolver/                         # NEW module
...
└── portunus-server/src/serve.rs           # wire new metric into the registry (one-line)
```

**Structure Decision**: same multi-crate workspace as v0.1.0 / v0.2.0;
the only structural addition is the `portunus-client/src/resolver/`

...
recent `cargo bench -p forward-client --bench data_plane` run, and exits
non-zero if any benchmark's median regressed beyond `--max-regression-pct`
versus `crates/forward-client/benches/baselines/v0.1.0.json`.

Usage:
    cargo bench -p forward-client --bench data_plane -- --save-baseline new
...
BASELINE = ROOT / "crates/forward-client/benches/baselines/v0.1.0.json"

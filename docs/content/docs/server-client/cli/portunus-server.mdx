...
loopback HTTP API, so they require `PORTUNUS_OPERATOR_TOKEN` to be set. Its
value is the static `operator_token` from `server.toml` — generate one with
[`gen-token`](#server-lifecycle) and paste it into the config:
...
| Subcommand | Purpose |
| --- | --- |
| `user-add <id> --display-name <name> [--role admin\|user] --password-stdin` | Add a user (`--role` defaults to `user`). An initial password is required — pipe it via `--password-stdin` |
| `user-list` | List users |
| `user-get <id>` | Show user details |
| `user-remove <id>` | Remove user (cascades grants + rules) |
| `reset-password <user_id> [--temporary] [--password-stdin]` | Local password recovery against the data dir |

`user-add` requires an initial password; pipe it on the first stdin line with
`--password-stdin`:
...
reads the new password from the first stdin line for automation. Password reset
revokes the user's active Web sessions.
...
| --- | --- |
| 0 | Success |
| 1 | Generic error (config, IO, `store_in_use`, …) |
| 2 | Validation error |

...
A pin mismatch surfaces as a TLS handshake failure — there is no
dedicated pin event. The client logs `control.connect_failed` with the
certificate/TLS detail in its `error=` field and **retries with backoff**
(it does not exit):

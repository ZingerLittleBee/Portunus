...
- **v1.3 (Linux `splice(2)` fast path)** — TCP throughput roughly
  ...
  see [Troubleshooting → Disabling the Linux fast path](/en/docs/server-client/operations/troubleshooting#disabling-the-linux-fast-path-for-triage).
- **v1.4 (traffic quotas)** — adds SQLite migration `V008`
  ...
  [Traffic Quotas runbook](/en/docs/server-client/operations/runbook-traffic-quotas).

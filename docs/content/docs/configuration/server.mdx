---
title: Server Configuration
description: Every setting in server.toml, with ranges and defaults.
icon: ServerCog
---

`forward-server` reads its config from `server.toml` in the directory
passed via `--config-dir`. A minimal config that uses only defaults is
valid — every key below is optional.

## Listeners

```toml
# gRPC control-plane listener — clients dial this address.
# Bind to 0.0.0.0 for cross-host clients, or to a specific interface.
control_listen = "0.0.0.0:7443"

# Operator HTTP API + Web UI. MUST stay on loopback by default.
# Every /v1/* request also requires Authorization: Bearer <token>,
# so the loopback bind is defence-in-depth.
operator_http_listen = "127.0.0.1:7080"

# Prometheus /metrics endpoint. Same loopback constraint.
metrics_listen = "127.0.0.1:7081"
```

## TLS material

```toml
# Self-signed material is auto-generated on first run if missing.
# The cert SAN includes hostname + "localhost" + 127.0.0.1 only —
# for routable deployments, replace with operator-managed materials
# whose SAN covers the address(es) clients will dial.
# server.key MUST be mode 0600 (the loader rejects looser perms).
tls_cert_path = "/var/lib/forward/server.crt"
tls_key_path  = "/var/lib/forward/server.key"
```

## Persistence

```toml
# Bearer-token store for client authentication.
# Created on first `provision-client`. Mode 0600.
# Tokens are 256-bit random secrets; keep this file backed up.
token_store_path = "/var/lib/forward/tokens.json"

# Operator-side identity store (users / credentials / grants).
# Mode 0600. Atomic-write JSON; safe to rsync across hosts.
operator_store_path = "/var/lib/forward/identity.json"
```

Since v0.8 most state lives in `<data-dir>/state.db` (SQLite). See
[SQLite Storage](/docs/features/sqlite-storage).

## Bootstrap shortcut

```toml
# One-shot bootstrap shortcut for the operator superadmin.
# When set AND the identity store has NO superadmin yet, the server
# auto-mints the reserved `_legacy` superadmin with this exact token
# on first start. After bootstrap, leave the line in (silently
# ignored once a superadmin exists) or remove it.
operator_token = "<43-char URL-safe-base64 token>"
```

Generate a token: `forward-server gen-token`.

Alternative: `forward-server bootstrap-superadmin --name ops` prints
a fresh bearer once.

## Drain & logging

```toml
# Graceful drain timeout for forwarded connections on SIGINT/SIGTERM.
# In-flight rules get up to this many seconds before kernel reaps.
# Match the systemd unit's TimeoutStopSec to avoid kill -9 mid-drain.
shutdown_drain_timeout_secs = 30

# "json" (default — structured) or "compact".
log_format = "json"
```

## Range rules

```toml
# Maximum ports a single port-range rule may span.
# Each port consumes one TCP listener (one fd) on the client.
# Default: 1024. Raise only after raising LimitNOFILE in the systemd unit.
range_rule_max_ports = 1024
```

## UDP tunables

```toml
# Idle window before a per-flow upstream socket is reaped.
# Range: 30..=300. Default: 60.
udp_flow_idle_secs = 60

# Per-rule cap on simultaneous live UDP flows.
# Range: 1..=65535. Default: 1024.
# Sustained churn beyond this bumps forward_rule_flows_dropped_overflow_total
# rather than evicting existing flows.
udp_max_flows_per_rule = 1024
```

## DNS resolver

The current release bakes the spec-fixed defaults into `ResolverConfig`
(client-side); they are not yet exposed as operator overrides:

| Knob | Value |
| --- | --- |
| `cache_floor` | 5 s |
| `cache_ceiling` | 5 min |
| `stale_while_error_grace` | 30 s |
| `attempt_timeout` | 3 s |
| `negative_cache_retry` | 3 s |
| `max_concurrent_resolves` | 64 |

A future release may expose these under a `[resolver]` table.

## CLI overrides

Every listen address is also overridable on the CLI:

```sh
forward-server --config-dir ./srv \
  --control-listen 0.0.0.0:7443 \
  --operator-http-listen 127.0.0.1:7080 \
  --metrics-listen 127.0.0.1:7081 \
  serve
```

CLI flags take precedence over `server.toml`.

## Example: production-ready server.toml

```toml
control_listen        = "0.0.0.0:7443"
operator_http_listen  = "127.0.0.1:7080"
metrics_listen        = "127.0.0.1:7081"

tls_cert_path         = "/etc/forward-rs/server.crt"
tls_key_path          = "/etc/forward-rs/server.key"

token_store_path      = "/var/lib/forward-rs/tokens.json"
operator_store_path   = "/var/lib/forward-rs/identity.json"

shutdown_drain_timeout_secs = 30
log_format = "json"

range_rule_max_ports   = 1024
udp_flow_idle_secs     = 60
udp_max_flows_per_rule = 1024
```

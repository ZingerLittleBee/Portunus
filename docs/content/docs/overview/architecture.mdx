...
[`portunus-standalone`](/en/docs/standalone/configuration) forwarder consume —
...
[`portunus-standalone`](/en/docs/standalone/configuration) reads a TOML file
...
[standalone guide](/en/docs/standalone/configuration) for the full schema.
...
- **TCP**: each rule binds an `accept` loop; per-connection bidirectional
  ...
  [performance report](/en/docs/overview/performance).

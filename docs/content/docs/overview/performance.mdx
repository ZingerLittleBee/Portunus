...
[Performance History](/en/docs/overview/performance-history) page.
...
the [v0.11 / v1.3.0 baselines](/en/docs/overview/performance-history).
...
[v0.11 baseline](/en/docs/overview/performance-history)
...
- Per-connection setup latency, half-close semantics, byte counters,
  ...
  [Rate Limiting & QoS — Interaction with the splice fast path](/en/docs/server-client/features/rate-limiting#interaction-with-the-splice-fast-path).
...
- The operator surface is unchanged: no new rule field, no wire-protocol
  ...
  testing (see
  [Disabling the Linux fast path for triage](/en/docs/server-client/operations/troubleshooting#disabling-the-linux-fast-path-for-triage)).
...
[Performance History](/en/docs/overview/performance-history).

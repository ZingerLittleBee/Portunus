...
| Status | Body | Cause |
| --- | --- | --- |
| `401 unauthenticated` | `{"error":"unauthenticated"}` | Missing / invalid bearer or Web session |
| `403 not_owner` | `{"error":"not_owner"}` | Authenticated, but caller does not own the resource |
| `403 password_change_required` | `{"error":"password_change_required"}` | Web session must change a temporary password first |

---
title: Installation
description: Install portunus-server and portunus-client with the one-line script, Docker Compose, or a source build.
icon: Download
---
...
`/usr/local/bin`. See [Client Configuration](/en/docs/configuration/client#install-the-binary)
for flags (`--version`, `--bin-dir`, `--systemd`).
...
Ask an operator for an enrollment command (Web UI **Clients** page, or
`portunus-server enroll-client <name>`), then on the edge host:
...
See [Deploy with Docker](/en/docs/deployment/docker) for the full client
container reference, backup, and production notes.
...
`cd deploy/systemd && sudo ./install.sh` instead (the binaries must already be
on `PATH`).

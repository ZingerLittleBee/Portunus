...
From the Web UI **Clients** page (or the `provision-client` CLI), generate a client
bundle. It already embeds the advertised endpoint (the TCP Proxy `host:port`) and
the pinned cert fingerprint, so `portunus-client` dials the TCP Proxy directly.

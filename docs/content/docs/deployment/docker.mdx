...
Host networking keeps the Web UI and operator API on the server host's
loopback interface at `http://127.0.0.1:7080/`, while the client control-plane
listener remains reachable on `0.0.0.0:7443`.

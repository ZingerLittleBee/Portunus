...
to the `enroll` command too, or set a reachable
[advertised endpoint](/en/docs/features/advertised-endpoint). And confirm the
`enroll` step wrote `client.bundle.json` **before** the long-lived `docker run`:
a missing bind-mount source path is created by Docker as a **directory**, which
the client rejects with `Is a directory`.
</Callout>
...
`install.sh client --deploy docker` writes exactly this `compose.yml` for
you — enroll once first so `./client.bundle.json` exists (it refuses
otherwise). Binary and enroll details:

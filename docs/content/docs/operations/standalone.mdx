...
- Pin a specific version in production (`:1.5.0` rather than `:latest`).
...
When `--config` is omitted the binary searches `$PORTUNUS_STANDALONE_CONFIG`,
then `./portunus.toml`. Exit 2 if neither resolves.

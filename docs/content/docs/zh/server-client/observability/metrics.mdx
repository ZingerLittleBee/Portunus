...
指标。该端点仅绑定 loopback —— 请用同主机的兄弟 Prometheus 或 sidecar 抓取。
...
规则移除时，大多数 per-rule 行随之移除；累计字节计数器
（`portunus_rule_bytes_in_total` / `portunus_rule_bytes_out_total`）按
Prometheus 惯例保留。
...
的跨规则汇总）将 `rule` 置为空（`rule=""`）、`owner` 仍填充。三个 collector 都
是如此，包括 `portunus_rate_limit_active_connections` —— 它同样会发出
`rule=""` 的所有者聚合行。用 `{rule!=""}` 筛选逐规则行，用 `{rule=""}` 筛选
所有者聚合行。
...
这些指标以 `{user, client}` 为键 —— 不是 `owner` —— 用于跟踪按 (user, client)
维度的月度字节预算。`bytes_used` 是当期已消耗的累计字节，`bytes_limit` 是预算。
`portunus_traffic_quota_exhausted` 是 gauge（配额当前耗尽时为 `1`，否则为
`0`）。`period_resets_total` 统计周期边界翻滚次数，`exhausted_total` 统计首次
当期耗尽次数。

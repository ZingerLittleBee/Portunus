...
API bearer tokens still exist, but they are separate from Web login. Use API
tokens for CLI and automation (`PORTUNUS_OPERATOR_TOKEN`); use passwords for
the browser.
...
- Session cookies are `HttpOnly`, `SameSite=Lax`, and `Secure` when TLS is
  enabled.
- API tokens and temporary passwords are shown only in one-time reveal dialogs
  and are never placed in URLs.
...
Web sessions and revokes API tokens by default unless the admin explicitly
keeps them.

If the last `superadmin` forgets the password, recovery is local-only:

...
On reverse-proxied / PaaS deployments (Railway, fly.io, …) the
public host the client must dial is not the host the server binds
locally, and it can differ between environments. Instead of baking it
into an environment variable only, the endpoint is resolved at runtime
through a tiered, fail-closed resolver and can be set by an operator
without redeploying.

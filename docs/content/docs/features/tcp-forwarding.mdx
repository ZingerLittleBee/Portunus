---
title: TCP Forwarding
description: The core forwarding primitive — bind a TCP listener on the edge client and forward every accepted connection to a configured upstream target.
icon: ArrowRightLeft
---

TCP forwarding is the project's bread-and-butter. Every other feature
(UDP, port ranges, SNI routing, rate limiting…) layers on top of it.

## How it works

Each TCP rule activates a single `accept` loop on the client. For every
accepted connection the client opens one upstream `TcpStream::connect`
to the configured target and runs a bidirectional byte copy until either
side closes.

There is **no userspace buffering** beyond the kernel's send/recv
windows. The forwarder is a pure L4 byte passthrough — TLS sessions,
HTTP requests, and arbitrary protocols pass through unmodified.

## Push a rule

```sh
forward-server push-rule <client_name> <listen_port> <target_host>:<target_port>
```

Example:

```sh
forward-server push-rule edge-01 8080 example.com:80
# → rule_id=1
```

Within ~1 second (the ack budget) the rule transitions
`Pending → Active` and the client starts accepting on port 8080.

## Inspect

```sh
# Operator CLI
forward-server list-rules --client edge-01
forward-server rule-stats 1

# Operator HTTP API
curl -H "Authorization: Bearer $FORWARD_OPERATOR_TOKEN" \
     http://127.0.0.1:7080/v1/rules

# Prometheus
curl -s http://127.0.0.1:7081/metrics | grep '^forward_rule'
```

## Remove

```sh
forward-server remove-rule <rule_id>
```

In-flight connections drain up to `shutdown_drain_timeout_secs`
(default 30 s). The listener stops accepting immediately.

## Failure modes

| Condition | Outcome |
| --- | --- |
| Listen port already in use on client | Rule state `Failed(port_in_use)`; remove + retry on a free port |
| Target connect refused / timed out | Connection-level error; rule stays `Active`, no metric scrubbing |
| Client disconnected when rule pushed | HTTP 4xx with `client_not_connected` |
| Client reconnects | Rules persist server-side and re-push to the same client |

## Metrics

Per-rule collectors (one row per live rule, labels `{client, rule, owner}`):

- `forward_rule_bytes_in_total`
- `forward_rule_bytes_out_total`
- `forward_rule_active_connections`

See [Metrics](/docs/observability/metrics) for the full collector list.

## See also

- [UDP Forwarding](/docs/features/udp-forwarding) — same model for UDP
- [Multi-target Failover](/docs/features/multi-target-failover) — push 1..=8 targets per rule
- [DNS-name Targets](/docs/features/dns-targets) — use a hostname instead of an IP

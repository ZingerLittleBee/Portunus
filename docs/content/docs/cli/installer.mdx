...
`config` and `env` accept only these keys (anything else is a hard
error):
...
flags actually live: the systemd `ExecStart=` override drop-in (binary)
or the compose `command:` array (docker). It **hydrates the current
values first**, so changing one key preserves the others, then offers a
service restart (`up -d` recreates the container for docker). `get` reads
the persisted flag value back (`<unset>` if none); `env` dumps all keys.

For a **docker** deploy, `data-dir` is not settable — it maps to the
container's fixed `/var/lib/portunus` volume mount; edit `compose.yml`
directly if you must relocate it.

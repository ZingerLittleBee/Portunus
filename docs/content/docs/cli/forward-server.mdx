...
`forward-server` is the control-plane binary. Most subcommands hit the
loopback HTTP API, so they require `FORWARD_OPERATOR_TOKEN` to be set.

```sh
export FORWARD_OPERATOR_TOKEN=<paste-token-here>
```

Run `forward-server --help` (or `forward-server <subcommand> --help`)
...
| `restore --in <path> [--force]` | Restore a backup; runs forward migrations |
...
| 4 | `FORWARD_OPERATOR_TOKEN` missing |
...
| Variable | Purpose |
| --- | --- |
| `FORWARD_OPERATOR_TOKEN` | Bearer for every operator subcommand |

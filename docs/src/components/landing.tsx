...
const copy = {
  en: {
    ...
    headlineAccent: "managed from one place.",
    hero: "Run a lightweight client on each public-facing host, and manage every TCP/UDP rule from one server. Permissions, rate limits, metrics, and audit logs come built in — and your traffic flows through the client, never the server.",
    primary: "Read the docs",
    secondary: "Performance report",
    proof: "Linux · Rust · single static binary",
    ...
    statusOk: "OK",
    statusActive: "ACTIVE",
    rangesTitle: "Benchmarks you can rerun yourself.",
    rangesIntro:
      "The report lists the test host, the exact commands, the raw results, and the caveats — run it again and compare.",
    ranges: [
      [
        "100M – 20G",
        "Uncapped TCP keeps pace with the kernel's own iptables, end to end.",
      ],
      [
        "Standalone / Client",
        "Both builds run the identical forwarding core — no throughput penalty either way.",
      ],
      [
        "Rate-limited rules",
        "Capped rules keep every metric, counter, and audit event accurate.",
      ],
    ],
    ...
    capabilities: [
      {
        icon: Network,
        title: "Rules",
        text: "Forward TCP or UDP — single ports or whole ranges, to IP or DNS targets. Route HTTPS to different backends by SNI hostname, without decrypting it.",
      },
      {
        icon: ShieldCheck,
        title: "Access",
        text: "Grant each user only the clients, protocols, and port ranges they need. The server checks ownership before any rule goes live.",
      },
      {
        icon: SlidersHorizontal,
        title: "Limits",
        text: "Cap bandwidth, new connections, and concurrent connections per rule or per user — plus monthly traffic quotas that cut off at the limit.",
      },
      {
        icon: Activity,
        title: "Visibility",
        text: "Prometheus metrics, structured logs, an audit trail, embedded SQLite state, and built-in backup and restore.",
      },
    ],
    ...
    workflowTitle: "Four steps to live traffic.",
    workflow: [
      "Generate a one-time enrollment command on the server.",
      "Run the client on the host that holds your public ports.",
  ...
  zh: {
    title: "Portunus",
    headlineLead: "TCP / UDP 高速转发，",
    headlineAccent: "集中管理",
    hero: "在每台公网入口机器上运行轻量 Client，所有 TCP / UDP 规则都从一台 Server 统一下发。权限、限速、指标、审计全部内置 —— 业务流量只经过 Client，不经过 Server。",
    primary: "阅读文档",
    secondary: "性能报告",
    proof: "Linux · Rust · 单文件静态二进制",
    ...
    capabilities: [
      {
        icon: Network,
        title: "规则",
        text: "转发 TCP 或 UDP，单端口或整段端口范围，目标可以是 IP 或域名。HTTPS 按 SNI 域名分流到不同后端，全程不解密。",
      },
      {
        icon: ShieldCheck,
        title: "权限",
        text: "只给每个用户分配他需要的 Client、协议和端口范围。Server 在规则生效前先校验归属。",
      },
      {
        icon: SlidersHorizontal,
        title: "限额",
        text: "按规则或按用户限制带宽、新连接速率和并发连接数，还能设置月度流量配额，到额自动停。",
      },
      {
        icon: Activity,
        title: "可观测",
        text: "Prometheus 指标、结构化日志、审计记录、内置 SQLite 状态，以及备份 / 恢复。",
      },
    ],
    topologyTitle: "一台 Server\n管理多台转发机器",
    topologyIntro:
      "你用 CLI、API 或 Web UI 在 Server 上改规则，Server 通过固定证书的 TLS 把签名规则下发给持有公网端口的 Client。业务流量只走 Client，不经过 Server。",
    nodeOperator: "你",
    nodeServer: "Server",
    nodeServerSub: "规则 + 权限",
    nodeClient: "Client",
    ...
    edgeControl: "规则",
    edgeData: "业务流量",
    workflowTitle: "四步上线转发。",
    workflow: [
      "在 Server 上生成一次性的 Client 注册命令。",
      "在持有公网端口的机器上运行 Client。",
      "通过 CLI、HTTP API 或 Web UI 添加转发规则。",
      "实时查看流量、配额和被拒绝的请求。",
    ],
    finalTitle: "先跑通一条转发规则",
    finalText:
      "只有一台机器？用 standalone，一个 TOML 文件搞定。多台机器？再加上 Server 和 Client —— 底层是同一套转发代码。",
    finalCta: "打开文档",
  },
} as const;
